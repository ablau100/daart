...
from daart.models.pooling import PMA, LinearDimReducer, MAP, AddPositionalEncoding, TW, FPE, RE
...
class Segmenter(BaseModel):
    ...
    def __init__(self, hparams):
        ...
        if hparams.get('use_pool', False):
            ...
            in_d = hparams['pool']['in_d']
            out_d = hparams['pool']['out_d']
            #self.model['pool'] = PMA(dim=D, num_heads=H, num_seeds=S)#, ln=True)
            #self.model['pool'] = PMA(in_d, H, D, out_d)
            #self.model['pool'] = LinearDimReducer(in_d, out_d)
            #self.model['pool'] = MAP(in_d, H, out_d)
            # override input_size for subsequent TCN
            hparams['input_size'] = in_d#out_d #D * S

            #self.model['pool'] = AddPositionalEncoding(548, 768)
            self.model['pool'] = RE(10, 768)
    ...
    def training_step(self, data, accumulate_grad=True, **kwargs):
        ...
        #print('pres', labels_strong_reshape[labels_strong>0].detach().cpu().numpy().argmax(axis=1))
        ...
        return loss_dict
         ...
         # if we're not doing external AMP‐driven backward, do the old in-step backward
        #external_backprop = self.hparams.get('external_backward', False)
        #if accumulate_grad and not external_backprop and loss.item() != 0:
         #   loss.backward()

        # return raw loss Tensor for GradScaler, plus a scalar for metrics
        #loss_dict['loss']       = loss              # ← Tensor
        #loss_dict['loss_value'] = loss.item()       # ← float for logging
        #return loss_dict
